...
/**
 * This has been split off from OfflineFileServer so that OfflineFileServer can
...
public class OfflineWavFileServer extends OfflineFileServer<FileDataMapPoint> {
	...
	/**
	 * Check for command line arguments, which will be important when running batch
	 * offline tasks that will need to set where wav files are
	 */
	private void checkCommandLine() {
		String arg = CommandLine.getCommandLine().getCommandParameter(FolderInputSystem.GlobalWavFolderArg);
		if (arg != null) {
			getOfflineFileParameters().folderName = arg;
			System.out.printf("Updating offline wav folder to %s\n", arg);
		}
	}
	...
	public void createOfflineDataMap(Window parentFrame) {
		/**
		 * Do some checks and if this is being controlled by the acquisition and the
		 * acquisition is set to folder input, then set the params to be the daq folder.
		 */
		checkDaqParams();
	...
	public FileDataMapPoint createMapPoint(File file, long startTime, long endTime) {
		...
		// need to see if it's an xwav file from a HARP that would contain additional
		// header data.
		...
		if (harpHeader == null || harpHeader.harpCycles == null) {
			...
			for (int i = 0; i < cycles.size(); i++) {
				 ...
				 * here, then return null so that the functions in the main map maker
				 * (OfflineFileServer) doesn't add anything itself.
				 */
				FileSubSection subSection = new FileSubSection(harpCycle.getByteLoc(),
						harpCycle.getByteLoc() + harpCycle.getByteLength());
				...
				FileDataMapPoint mapPoint = new FileDataMapPoint(harpFile, harpCycle.gettMillis(),
						harpCycle.getEndMillis());
...
//			e.printStackTrace();
	...
	public void sortMapEndTimes(OfflineFileServer.MapMaker mapMaker) {
		...
		while (it.hasNext()) {
			...
			String msg = String.format("Checking file %d of %d: %s", nDone, totalPoints,
					mapPoint.getSoundFile().getName());
			...
			if (mapPoint.getMatchedPoint() != null) {
				...
				/*
				 * There was a bug #283, not entirely understood where wav files were being set
				 * with the same end and start times. I think this is because they are initially
				 * listed that way and then updated, but for some reason the update hadn't
				 * worked. In any case, this will solve it, forcing it to re-get the wav
				 * duration if the current duration is zero.
				 */
	...
	public boolean loadData(PamDataBlock dataBlock, OfflineDataLoadInfo offlineLoadDataInfo,
		...
		// Debug.out.println(String.format("Request raw data from %s to %s",
		// PamCalendar.formatDateTime(dataStart),
		// PamCalendar.formatTime(dataEnd)));
		/*
		 * Find the data mapped files, work through them and load data as appropriate,
		 * forming into standard sized data units and passing off into the dataBlock.
		 * 
		 * ***** Note that this function doesn't clear data units and probably isn't
		 * meant to so that subsequent calls can be made. However, this is quite risky !
		 * So unless you're very sure that it's going to be called for sequential block
		 * of data make sure you call clearall for the datablock in question to avoid
		 * calamity.
		 */
		OfflineDataMap<FileDataMapPoint> dataMap = getDataMap();
		FileDataMapPoint mapPoint = findFirstMapPoint(dataMap.getListIterator(), offlineLoadDataInfo.getStartMillis(),
				offlineLoadDataInfo.getEndMillis());
		if (mapPoint == null) {
			return false;
		}

		/**
		 * This just gets a loader based on the types of audio file. These are a bit
		 * different for wav, flac, and SUD files. The loading itself of multiple map
		 * points is handled a bit later in the call to loadAudioData, which will
		 * iterate again through the file map.
		 */
		PamAudioFileLoader audioFile = PamAudioFileManager.getInstance().getAudioFileLoader(mapPoint.getSoundFile());

		if (audioFile == null) {
			System.err.println("OfflineWavFileServer: could not find audio loader for mapped sound file: "
					+ mapPoint.getSoundFile());
			return false;
		}
